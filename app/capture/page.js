...
export default function CapturePage() {
  ...
  useEffect(() => {
    async function setupLiveView() {
      ...
      } catch (err) { console.error("Không tìm thấy webcam ảo:", err); }
    }
    setupLiveView();
  ...
  const triggerCapture = async () => {
    ...
    try {
      // 1. Gửi lệnh chụp và xử lý (Phải await ở đây để đảm bảo ảnh đã được magick xử lý xong trước khi hiện preview)
      ...
      if (data.success) {
        ...
        // 2. Cập nhật ảnh thật từ API route vừa tạo ở Bước 1
        // Thêm ?t=... để trình duyệt không cache ảnh cũ
        setLastPhoto(`/api/photo/${fileName}?t=${ts}`);
      }
    } catch (err) {
      console.error("Lỗi chụp ảnh thật:", err);
    } finally {
    ...
    <div className="flex flex-col h-screen max-h-screen bg-mochi-bg text-mochi-gold p-6 select-none overflow-hidden font-serif relative">
      ...
      <div className="flex flex-1 gap-6 min-h-0"> 
        <div className="flex-[4] relative border-2 border-mochi-gold bg-black rounded-sm overflow-hidden flex items-center justify-center">
          ...
          {isProcessing && <div className="z-20 bg-mochi-gold text-mochi-bg px-10 py-5 font-black text-2xl uppercase">Đang xử lý...</div>}
          {!isCounting && !isProcessing && !showConfirmExit && (
            <button onClick={() => setIsCounting(true)} className="z-10 w-24 h-24 rounded-full border-4 border-mochi-gold bg-mochi-bg/20 flex items-center justify-center active:scale-90 transition-transform">
        ...
        <div className="flex-[2] flex flex-col border-2 border-mochi-gold p-4 bg-black/20 overflow-hidden">
          ...
          <div className="flex-1 flex items-center justify-center overflow-hidden">
            {lastPhoto ? (
                <img src={lastPhoto} alt="preview" className="max-w-full max-h-full object-contain border-4 border-white shadow-xl animate-in fade-in duration-500" />
            ) : (
                <div className="text-[10px] uppercase opacity-20 text-center italic">Sẵn sàng</div>
            )}
          </div>
          <button onClick={() => { setShowConfirmExit(true); setExitCountDown(5); }} className="mt-6 py-4 border-2 border-mochi-gold text-mochi-gold font-black text-xs uppercase active:bg-mochi-gold active:text-mochi-bg transition-colors">
            Kết thúc
          </button>
        </div>
      </div>
      ...
      {showConfirmExit && (
        <div className="absolute inset-0 z-50 bg-black/95 flex items-center justify-center p-10">
          <div className="max-w-xl w-full border-4 border-mochi-gold p-12 text-center bg-mochi-bg">
            <h2 className="text-4xl font-black uppercase italic mb-6">Kết thúc chụp?</h2>
            <div className="flex flex-col gap-5">
              ...
              <button onClick={() => setShowConfirmExit(false)} className="py-6 text-2xl font-black uppercase text-mochi-gold border-2 border-mochi-gold rounded-full active:bg-mochi-gold active:text-mochi-bg">Quay lại</button>
            </div>
          </div>
        </div>
      )}
